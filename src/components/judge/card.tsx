...
function JudgeCard({ name, title, image, instagram, linkedin, twitter, profession, setActiveJudge }: JudgeCardProps) {
  ...
  return (
    <motion.div
      ...
      <Image src={image} width={500} height={500} alt={name} className="object-cover rounded-xl aspect-square w-7/10 h-7/10 absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 group-hover:h-4/5 transition-all duration-500 ease-in-out group-hover:-translate-y-2/3 " />
      <motion.div layout className="relative w-full flex flex-col items-center top-[calc(90%)] md:top-[calc(100%)] -translate-y-full h-fit gap-2">
        <motion.h3 className="text-lg uppercase" >{name}</motion.h3>
        <motion.p className="text-sm text-slate-500" >{title}</motion.p>
        <motion.h3 className="text-sm font-medium text-center" >{profession}</motion.h3>
        <motion.div layout className="grid w-full" style={{ gridTemplateRows: isOn ? 'minmax(0, 1fr)' : '0', opacity: isOn ? 1 : 0 }}>
          <motion.div layout className="text-accent-secondary min-h-0 w-full flex flex-col gap-1 text-sm">
            <motion.a layout href={instagram} className='flex gap-2 items-center' style={{ width: isOn ? '100%' : 0 }} transition={{ delay: 0.4 }}
              target='_blank' ><InstagramIcon strokeWidth={1.25} size={16} />Instagram</motion.a>
            <motion.a layout style={{ width: isOn ? '100%' : 0 }} transition={{ delay: 0.3 }}
              href={linkedin} className='flex gap-2 items-center' target='_blank'><LinkedinIcon strokeWidth={1.25} size={16} />LinkedIn</motion.a>
            <motion.a layout style={{ width: isOn ? '100%' : 0 }} transition={{ delay: 0.2 }} href={twitter} className='flex gap-2 items-center' target='_blank'><TwitterIcon strokeWidth={1.25} size={16} />Twitter</motion.a>
          </motion.div>
        </motion.div>
