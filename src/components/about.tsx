...
export default function About() {
  const objectives = ["1. To stimulate and harness innovative solutions for pressing sustainability and society challenges in Southeast Asia.", "2. To generate actionable insights that can inform and shape environmental and social policy and governance in the region.", "3. To develop and refine solutions that are not only effective but also scalable across different contexts within Southeast Asia."]
  const outcomes = [
  ...
  const gtks = [{
    ...
    title: "Outcomes",
    arr: outcomes,
    highlight: "Outcome of this initiative is to influence mid-to-long-term policy, and to promote successful pilots scaling to national/ regional levels."
  }, {
    title: "Stunting Prevention Solutions",
    arr: stunting,
    highlight: "Innovative solutions to prevent stunting in Southeast Asia"
  }, {
    title: "Urban Farming Solutions",
    arr: urbanFarming,
    highlight: "Optimization of Community Urban Farming Practices and Commercialization"
  }, {
    title: "Tidal Floods Solutions",
    arr: tidalFloods,
    highlight: "Development of Adaptation / Mitigation Solutions Towards Worsened Tidal Floods Problem"
  }]
        ...
        {/* <div className='grid md:grid-cols-3 gap-8 items-center'>
          <div className='space-y-6 col-span-2'>
            ...
            <div className='relative backdrop-blur-sm p-6 rounded-lg'>
              ...
              <WordsList title='Objectives' points={objectives} />
            </div>
          </div>
          <div className='relative hidden md:block'>
            ...
            <div className='relative z-10 rounded-lg p-4'>
              <div className='relative group'>
                <Header className='mb-4 md:text-2xl font-medium'>Here are our objectives for holding the event</Header>
                <div className='w-20 transition-all group-hover:w-44 h-1 bg-green-500'></div>
              </div>
            </div>
          </div>
        </div> */}
        {gtks.map((obj, index) => <GTK key={obj.highlight} index={index} {...obj} />)}
      </div>
    </section>
  );
}

function GTK({ highlight, arr, index, title }: { highlight: string, arr: string[], index: number, title: string }) {
  const isOdd = index % 2 === 0
  return (
    <div className={cn('grid md:grid-cols-3 gap-8 items-center min-h-screen py-8 md:py-12', isOdd ? "[grid-template:'main_main_rest']" : "[grid-template:'rest_main_main']")}>
      <div className='space-y-6 col-span-2 [grid-area:main]'>
        {index === 0 && (<div className='relative md:hidden ml-4'>
          <Header className='mb-4'>Get To Know</Header>
          <div className='w-20 h-1 bg-green-500'></div>
        </div>)}
        <div className='relative backdrop-blur-sm p-6 rounded-lg'>
          <div className='absolute -top-2 -left-2 w-4 h-4 border-t-2 border-l-2 border-green-500'></div>
          <div className='absolute -bottom-2 -right-2 w-4 h-4 border-b-2 border-r-2 border-green-500'></div>
          <WordsList title={title} points={arr} />
        </div>
      </div>
      <div className='relative hidden md:block [grid-area:rest]'>
        ...
        <div className='relative z-10 rounded-lg p-4'>
          <div className='relative group'>
            <Header className='mb-4 md:text-2xl font-medium'>{highlight}</Header>
            <div className='w-20 transition-all group-hover:w-44 h-1 bg-green-500'></div>
          </div>
        </div>
      </div>
